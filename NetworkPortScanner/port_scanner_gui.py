...
def scan_ports(target, start_port, end_port, output_file, scan_results):
    ...
    if output_file:
        generate_report(target, start_port, end_port,
                        scan_results.get(1.0, tk.END))
...
def generate_report(target, start_port, end_port, scan_data):
    ...
    pdf.multi_cell(
        0, 10, f"Scan Results from Port {start_port} to {end_port}:\n{scan_data}")
...
def start_scan_thread(target, start_port, end_port, output_file, scan_results):
    global stop_scan
    stop_scan = False
    scan_thread = threading.Thread(target=scan_ports, args=(
        target, start_port, end_port, output_file, scan_results))
    scan_thread.start()
...
def setup_gui():
    ...
    output_file_checkbox = tk.Checkbutton(
        root, text="Generate Report", variable=output_file_var)
    ...
    # Start Scan Button
    start_button = tk.Button(root, text="Start Scan", command=lambda: start_scan_thread(
        target_ip.get(), int(start_port.get()), int(end_port.get()), output_file_var.get(), scan_results))
